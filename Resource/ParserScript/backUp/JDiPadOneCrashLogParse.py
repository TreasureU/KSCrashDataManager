...
说明区:
	...
	指令类似于 python JDiPadOneCrashLogParse.py <iPad/iPhone>  <arch> <baseAddr> <symbolAddr>
	arch 必须是以下取值: armv7 \ armv7s \ arm64 \ armv7f
	baseAddr 和 symbolAddr 必须是16进制数
	如果希望直接使用指令,可以参考如下:
	atos -arch armv7  -o ../SourceSymbol/Jdipad/Jdipad -l 962560 2666928
"""
...
if len(sys.argv) != 5:
    print "Please Use : python JDiPadOneCrashLogParse.py <iPad/iPhone> <arch> <baseAddr> <symbolAddr> , addr is decimal system"
    sys.exit(-1)
